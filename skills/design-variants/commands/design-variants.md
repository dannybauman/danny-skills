...
4. **User has picked a winner** — they say "go with bmad" or "merge the stitch version" or "I like variant X":
   Run `bash ${CLAUDE_PLUGIN_ROOT}/run.sh --pick <tool-name>` to merge the winner into main and clean up other worktrees.
...
## Available Tools
Read from `${CLAUDE_PLUGIN_ROOT}/scripts/tools.json`. Default: superpowers, frontend-design, bmad, stitch, gstack.
