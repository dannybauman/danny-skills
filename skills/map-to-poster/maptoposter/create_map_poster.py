...
THEMES_DIR = "themes"
FONTS_DIR = "fonts"
POSTERS_DIR = os.environ.get("OUTPUT_DIR", "output")
...
def create_poster(city, country, point, dist, output_file, output_format, width=12, height=16, country_label=None, name_label=None, include_forests=False, include_buildings=False):
    print(f"\nGenerating map for {city}, {country}...")

    # Determine layers to fetch
    layers = 3 # Streets, Water, Parks
    if include_buildings: layers += 1

    # Progress bar for data fetching
    with tqdm(total=layers, desc="Fetching map data", unit="step", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt}') as pbar:
        # 1. Fetch Street Network
        pbar.set_description("Downloading street network")
        # Ensure the bounding box covers the larger dimension of the poster
        compensated_dist = dist * (max(height, width) / min(height, width))
        ...
        # 3. Fetch Parks & Green Spaces
        pbar.set_description("Downloading parks")
        park_tags = {'leisure': 'park', 'landuse': 'grass'}
        if include_forests:
            # We use a list for the tag values so OSMnx fetches all of them
            park_tags.update({'landuse': ['forest', 'meadow'], 'natural': ['wood', 'scrub', 'heath']})

        parks = fetch_features(point, compensated_dist, tags=park_tags, name='parks')
        pbar.update(1)
        ...
        if include_buildings:
            ...
            buildings = fetch_features(point, compensated_dist, tags={'building': True}, name='buildings')
            pbar.update(1)

    if G is None and (water is None or water.empty) and (parks is None or parks.empty) and (buildings is None or buildings.empty):
        raise ValueError(f"No map data found for {city}, {country} within {dist}m. Try increasing the distance.")
    ...
    # Project graph/features if they exist
    crs = "EPSG:3857" # Default metric projection if no graph
    G_proj = None
    if G is not None:
    ...
    if water is not None and not water.empty:
        ...
        if not water_polys.empty:
            ...
            try:
                water_polys = ox.projection.project_gdf(water_polys, to_crs=crs)
            except Exception:
    ...
    if parks is not None and not parks.empty:
        ...
        if not parks_polys.empty:
            ...
            try:
                parks_polys = ox.projection.project_gdf(parks_polys, to_crs=crs)
            except Exception:
    ...
    if buildings is not None and not buildings.empty:
        ...
        if not buildings_polys.empty:
            try:
                buildings_polys = ox.projection.project_gdf(buildings_polys, to_crs=crs)
            except Exception:
                buildings_polys = buildings_polys.to_crs(crs)
            # Use a slightly different color or standard gray
            building_color = THEME.get('buildings', '#D0D0D0')
            buildings_polys.plot(ax=ax, facecolor=building_color, edgecolor='none', zorder=2.5)

    # Layer 2: Roads (if they exist)
    if G_proj is not None:
        ...
        # If no road data, we still need to set the axis limits based on the requested area
        from shapely.geometry import Point as ShapePoint
        center_proj = ox.projection.project_geometry(ShapePoint(point[1], point[0]), to_crs=crs)[0]
        half_x = dist * (width / min(width, height))
        half_y = dist * (height / min(width, height))
        ax.set_xlim(center_proj.x - half_x, center_proj.x + half_x)
        ax.set_ylim(center_proj.y - half_y, center_proj.y + half_y)
        ax.set_aspect('equal')
...
Examples:
  ...
  python create_map_poster.py --list-themes
    ...
    parser.add_argument('--name', '--name-label', dest='name_label', type=str, help='Override city name text displayed on poster')
    parser.add_argument('--format', '-f', default='png', choices=['png', 'svg', 'pdf'],help='Output format for the poster (default: png)')
    parser.add_argument('--forests', action='store_true', help='Include detailed forest/woodland features')
    ...
    try:
        ...
        for theme_name in themes_to_generate:
            ...
            create_poster(args.city, args.country, coords, args.distance, output_file, args.format, args.width, args.height, country_label=args.country_label, name_label=args.name_label, include_forests=args.forests, include_buildings=args.buildings)
